...
(function ($, window) {
    ...
    var effect = {
        montage: function (data, pid, reg) {
            ...
            for (var i = 0; i < leng; i++) {
                if (data[i]['parentCatId'] == pid) {
                    ...
                    name = config.shorthand ? data[i].shortName : data[i].catName;
                    //反向：判断是否开启了简写，是就用输出简写，否则就输出全称
                    reverse = !config.shorthand ? data[i].catName : data[i].catName;
                    //存储的是数字还是中文
                    storage =  data[i].id ;

                    if (config.renderMode) {
                        //模拟
                        html += '<li class="caller" data-id="' + data[i].id + '" data-title="' + reverse + '" >' + name + '</li>';
                    } else {
                        //原生
                        html += '<option class="caller" value="' + storage + '" data-title="' + reverse + '" data-id="' + data[i].id + '">' + name + '</option>';
                    }
                }
            }
            html = data.length > 0 && html ? html : '<li class="forbid">您查找的没有匹配项</li>';
        ...
        seTemplet: function () {
            ...
            for (var i = 0; i < config.level; i++) { //循环定义的级别
                ...
                if (config.renderMode) {
                    //模拟
                    selectemplet += '<div class="col-xs-12  mr-10 mb-15 selector-item storey ' + citygrade + '" data-index="' + i + '">'
        ...
        search: function (event) {
            ...
            $.each(this.options.dataJson, function(key, value) {
                ...
                if(value.catName.search(inputVal) > -1 ||(value.id).toString().search(inputVal) > -1){
                    result.push(value);
                }
            });
    ...
    Citypicker.prototype = {
        init: function () {
            ...
            if (config.renderMode) {
                //模拟>添加数据
                $selector.find('.listing').data('id', '0').eq(0).find('ul').html(effect.montage.apply(self, [config.dataJson, '0']));
            } else {
                //原生>添加数据
                $selector.find('.province').append(effect.montage.apply(self, [config.dataJson, '0']));
            }
